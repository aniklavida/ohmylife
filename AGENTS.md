...
- **Planned for v1.0 — with a key, calls to that provider and to nothing else.** The in-site key path has no implementation, so nothing tests it. Write that assertion when the path lands, not before: a test written against nothing is the false green this file was added to remove.
